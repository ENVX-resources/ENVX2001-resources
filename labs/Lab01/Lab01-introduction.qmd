...
3.  Present their code and results using Quarto.
...
A skewness coefficient close to zero suggests the distribution is approximately symmetrical.
...
The data from Hobbs et al. (2024) comes as a csv file - which means `read_excel` will not work this time, so we need to install the `readr` package instead. Remember to do this in your console, not your coding script.
...
```{r}
Yellowstone <- read_csv('data/browsing_data_2003_2020_2.csv') # we will name this dataset 'Yellowstone'
```
...
::: callout-note
#### Sharpen your skills
Use `geom_line()` to make a line graph with `year` on the x-axis, and `site_full` on the y-axis.
