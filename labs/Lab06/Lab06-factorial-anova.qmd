...
The residual diagnostics show that the data is not normally distributed, but now has equal variance.  Given ANOVA is robust again non-normality, we will proceed with the analysis using the log transformed response variable.
...
par(mfrow=c(1,1))
```
The residual diagnostics show that the data is close to be normally distributed.  Given ANOVA is robust again non-normality, we will proceed with the analysis using the log transformed response variable.
