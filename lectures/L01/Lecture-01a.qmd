...
- Post on **Ed** for questions — we usually respond the same day (weekdays).
- **Drop-in sessions** (Zoom or in person) are available on request — just send us an email or post on Ed.
...
Researchers presented 18 pet dogs with food bowls at five locations, from "positive" (always rewarded) to "negative" (never rewarded). Dogs exposed to stressed human odours were slower to approach ambiguous locations — a sign of cognitive bias.
...
dogs$location <- factor(dogs$location,
  levels = c("P", "NP", "M", "NN", "N"),
  labels = c("Positive", "Near-pos", "Middle", "Near-neg", "Negative")
)

ggplot(dogs, aes(x = location, y = latency, fill = location)) +
  geom_boxplot(show.legend = FALSE) +
  labs(x = "Bowl location", y = "Latency to approach (s)") +
  theme_minimal(base_size = 18)
```

*You'll learn to compare multiple treatments with **ANOVA** (Weeks 3--6).*

## How are plants adapting to drought?

Capeweed populations collected across a natural aridity gradient in southern Australia were grown under controlled conditions to test whether plants from drier areas had evolved drought-resistant traits.
...
#| warning: false
#| message: false
plants <- read.csv("data/regression_plant-aridity_clean.csv")

ggplot(plants, aes(x = aridity, y = stomatal_conductance)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm") +
  labs(x = "Aridity index", y = "Stomatal conductance") +
  theme_minimal(base_size = 18)
```
